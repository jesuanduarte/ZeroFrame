...
namespace ZeroFrame.Application.Servicos
{
    ...
    public class UsuarioService : IUsuarioService
    {
        ...
        public async Task<UsuarioGetDto> AtualizarMeusDadosAsync(int usuarioId, UsuarioMeusDadosPutDto usuarioMeusDadosPutDto)
        {
            ...
            var emailNormalizado = usuarioMeusDadosPutDto.Email.Trim().ToLower();
            var usuarioMesmoEmail = await _usuarioRepository.ObterPorEmailAsync(emailNormalizado);

            if (usuarioMesmoEmail != null && usuarioMesmoEmail.Id != usuarioId)
                throw new BadRequestException("Este e-mail ja esta cadastrado.");

            usuario.Nome = usuarioMeusDadosPutDto.Nome.Trim();
            usuario.Email = emailNormalizado;
            usuario.Telefone = usuarioMeusDadosPutDto.Telefone.Trim();

            if (!string.IsNullOrWhiteSpace(usuarioMeusDadosPutDto.NovaSenha))
            {
                if (string.IsNullOrWhiteSpace(usuarioMeusDadosPutDto.SenhaAtual) ||
                    !SenhaValida(usuarioMeusDadosPutDto.SenhaAtual, usuario.Senha))
                {
                    throw new BadRequestException("Senha atual invalida.");
                }

                usuario.Senha = BCrypt.Net.BCrypt.HashPassword(usuarioMeusDadosPutDto.NovaSenha);
            }
