    ...
    public class CarrinhoService : ICarrinhoService
    {
        ...
        public async Task AtualizarAsync(CarrinhoPutDto carrinhoPutDto)
        {
            ...
            foreach (var itemDto in carrinhoPutDto.Itens)
            {
                ...
                itemAtualizado.PrecoUnitario = ProdutoPrecoService.CalcularPrecoFinal(variacao.Produto);
        ...
        private CarrinhoGetDto MapearCarrinhoGetDto(Carrinho carrinho)
        {
            ...
            var subtotal = itens.Sum(item => item.SubtotalOriginal);
            var desconto = itens.Sum(item => item.DescontoTotal);
        ...
        private static ItemCarrinhoGetDto MapearItemCarrinhoGetDto(ItemCarrinho item)
        {
            ...
            var precoOriginal = produto?.Preco ?? item.PrecoUnitario;
            var precoFinal = produto == null ? item.PrecoUnitario : ProdutoPrecoService.CalcularPrecoFinal(produto);
            var descontoUnitario = Math.Max(0m, precoOriginal - precoFinal);
            ...
            return new ItemCarrinhoGetDto
            {
                ...
                PrecoOriginalUnitario = precoOriginal,
                DescontoUnitario = descontoUnitario,
                Subtotal = item.Quantidade * precoFinal,
                SubtotalOriginal = item.Quantidade * precoOriginal,
                DescontoTotal = item.Quantidade * descontoUnitario
            };
        }
