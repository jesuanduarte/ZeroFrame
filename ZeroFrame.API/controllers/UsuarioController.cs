...
namespace ZeroFrame.API.Controllers
{
    ...
    public class UsuarioController : ControllerBase
    {
        ...
        [HttpPut("meus-dados")]
        public async Task<ActionResult<UsuarioGetDto>> AtualizarMeusDados(UsuarioMeusDadosPutDto usuarioMeusDadosPutDto)
        {
            var usuarioLogadoId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            if (!int.TryParse(usuarioLogadoId, out var usuarioId))
                return Unauthorized("Usuario nao identificado.");

            var usuarioAtualizado = await _usuarioService.AtualizarMeusDadosAsync(usuarioId, usuarioMeusDadosPutDto);
            return Ok(usuarioAtualizado);
        }

        // PUT: api/usuarios/{id}
        // Apenas Administrador usuario pode atualizar.
