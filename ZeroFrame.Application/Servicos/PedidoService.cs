    ...
    public class PedidoService : IPedidoService
    {
        ...
        public async Task CancelarAsync(int id)
        {
            await _unitOfWork.ExecuteInTransactionAsync(async () =>
            {
                ...
                var pagamento = await _pagamentoRepository.ObterPorPedidoIdAsync(pedido.Id);
                if (pagamento != null)
                {
        ...
        public async Task AtualizarStatusEntregaAsync(int pedidoId, PedidoStatusEntregaUpdateDto pedidoStatusEntregaUpdateDto, bool usuarioAdministrador)
        {
            ...
            else if (statusEntrega == "Em Separacao")
                pedido.Status = StatusPedido.PreparandoParaSaida;
            else if (statusEntrega == "Enviado")
                pedido.Status = StatusPedido.SaiuParaEntrega;
            else if (statusEntrega == "Entregue")
                pedido.Status = StatusPedido.Entregue;

            await SincronizarPagamentoComStatusEntregaAsync(pedido, statusEntrega);

            await _pedidoRepository.AtualizarAsync(pedido);
        }

        private async Task SincronizarPagamentoComStatusEntregaAsync(Pedidos pedido, string statusEntrega)
        {
            var pagamento = await _pagamentoRepository.ObterPorPedidoIdAsync(pedido.Id);
            ...
            if (statusEntrega is not ("Em Separacao" or "Enviado" or "Entregue"))
                return;
        ...
        private PedidosGetDto MapearPedidoGetDto(Pedidos pedido)
        {
            ...
            return new PedidosGetDto
            {
                ...
                StatusPagamento = ObterStatusPagamento(pedido),
        ...
        private static string ObterStatusPagamento(Pedidos pedido)
        {
            if (!string.IsNullOrWhiteSpace(pedido.Pagamento?.Status))
                return pedido.Pagamento.Status;

            return pedido.StatusEntrega switch
            {
                "Em Separacao" or "Enviado" or "Entregue" => "Pago",
                "Cancelado" => "Cancelado",
                _ => "Pendente"
            };
        }
        ...
        {
            // Usa a imagem cadastrada no produto; o front normaliza /uploads para a URL da API.
